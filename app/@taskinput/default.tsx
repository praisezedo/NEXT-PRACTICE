      ...
      <ul className="flex flex-col gap-5 mt-5 font-bold">
        {
            tasks.map((t : string , i : number) => {
                return <li className="flex justify-between gap-4" key={i + 1}><h1>{i + 1}  -   {t}</h1> <button className="border-2 rounded-tr-full border-black hover:animate-bounce transition-all duration-700s" onClick={() => handleDelete(i)}>×</button></li>
            })
        }
      </ul>
    </>
